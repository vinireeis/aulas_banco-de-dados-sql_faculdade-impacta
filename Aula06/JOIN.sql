...
SELECT 
	CONVERT(VARCHAR(6),DATA_EMISSAO,112) AS ANOMES,
	SUM(VLR_TOTAL) VALOR_FATURADO
INTO TB_VENDA_MENSAL
FROM TB_PEDIDO
GROUP BY CONVERT(VARCHAR(6),DATA_EMISSAO,112) 

SELECT * FROM TB_VENDA_MENSAL MES_BASE
 LEFT JOIN TB_VENDA_MENSAL MESES_POSTERIORES
  ON MES_BASE.ANOMES  IN (MESES_POSTERIORES.ANOMES -1, MESES_POSTERIORES.ANOMES-2)
 ORDER BY MES_BASE.ANOMES, MESES_POSTERIORES.ANOMES